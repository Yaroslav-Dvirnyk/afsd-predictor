...
# Языки интерфейса. Русский убран: программа подаётся на охранный документ
# и публикуется международно, английский — базовый. Строки ru в словарях
# оставлены — они не мешают и пригодятся, если язык вернут.
LANGS = [("en", "English"), ("uk", "Українська"), ("zh", "中文")]
...
TR = {
    ...
    "cbar_tpeak": dict(uk="Пікова температура", ru="Пиковая температура",
                       en="Peak Temperature"),
    "lbl_plotlang": dict(uk="підписи мовою інтерфейсу",
                         ru="подписи на языке интерфейса",
                         en="labels in interface language"),
...
GUIDE_TEXT["uk"] = """AFSD Predictor — ПОСІБНИК КОРИСТУВАЧА

1. ПРИЗНАЧЕННЯ
AFSD Predictor розраховує пікову температуру контакту процесу Additive Friction
Stir Deposition (AFSD) і будує карти режимів (2D-контур / 3D-поверхня) за
обраними параметрами, виділяючи вікно стабільності 0.6…0.9·T_solidus. Модель
ФОРС-КОНТРОЛЬОВАНА: осьова сила F і коефіцієнт тертя μ — вхідні дані.

2. ШВИДКИЙ СТАРТ
  1) Оберіть сплав прутка (Сплав).
  2) Оберіть процес: Shoulderless (круглий пруток) або Shouldered (квадратний
     брусок крізь шайбу).
  3) Задайте властивості прутка й підкладки, силу F, тертя μ та ККД η.
  4) У «Діапазони параметрів» позначте, які параметри варіюються («діапазон»),
     а які фіксовані («число»). Для карти потрібно ≥2 параметрів у «діапазоні».
  5) Оберіть осі X та Y і натисніть ▶ Побудувати.
  6) «Розрахунок у точці» — температура для конкретної точки.
  7) Матриця планування та рівняння регресії — у вкладці «Матриця».

3. ТИП ВХІДНИХ ДАНИХ
  • Shoulderless AFSD — круглий пруток. Фактори: ω, v_f (подача), v_t (траверс),
    D (діаметр прутка), F. Ширина валика = діаметр прутка (2R).
  • Shouldered AFSD (MELD) — квадратний брусок a×a подається крізь обертову
    шайбу із зазором H; радіус плями — з масового балансу. Фактори: ω, v_f, v_t,
    H (зазор), a (сторона бруска), F. Шайба відводить тепло вгору (свій матеріал).

4. МАТЕРІАЛИ
  • Список сплавів: вбудовані + ваші. «＋ Додати матеріал…» зберігає поточні
    властивості прутка під назвою.
  • Підкладка: «= матеріал прутка» (копіює пруток) або «своя підкладка» (вибір
    матеріалу / правка k, ρ, cₚ; «Зберегти підкладку…» — до бази).
  • Тертя μ пари: «μ = константа» або «μ(T) таблиця» (оберіть таблицю пари).
    «Таблиці μ(T)…» — редактор (додати / правити / видалити). «μ з моменту…»
    рахує μ = 3·M / (2·F·Rc) за крутним моментом і силою.
  • k, cₚ(T): позначка «T-залежність» вмикає табличні значення (самоузгоджена
    інтерполяція). «Бібліотека k, cₚ(T)…» — правка таблиць за матеріалами.

5. ПАРАМЕТРИ — ДІАПАЗОН чи ЧИСЛО
У кожного параметра перемикач: «діапазон» (min/max — входить до матриці й може
бути віссю) або «число» (одне фіксоване значення). Розмір плану = 3^N, де N —
кількість «діапазонних» параметрів: зафіксувавши 3 з 5, отримаєте невелику
матрицю (3² = 9 дослідів). На карті дві «діапазонні» величини йдуть на X/Y;
зайві отримують повзунок; «числові» тримаються сталими.
  • η — коеф. утримання тепла (0–1): калібрувальний коефіцієнт. η = 1 —
    адіабатична верхня оцінка; знижуйте η під виміряну температуру.
  • F — осьова сила [кН];  μ — коефіцієнт тертя пари.

6. КАРТА
  • 2D-контур або 3D-поверхня (Вигляд і зріз, або кнопки ▦ / ◳ на панелі).
  • Колір: «увесь діапазон» або «лише робоче вікно» (палітра всередині
    0.6…0.9·T_s, поза ним — білий фон).
  • Рамка перелічує фіксовані параметри; штрихові лінії — межі вікна.

7. РОЗРАХУНОК У ТОЧЦІ ТА ТОЧКИ НА КАРТІ
  • Введіть точку й натисніть «Розрахувати T_peak»: T (°C і K), статус вікна,
    H / R_eff, Pe, G, тиск p, дотичне τc і режим (ковзання / залипання) з
    переходом T*. Значок ⚠ — контакт біля солідусу (потрібен η < 1 або дод. стік).
  • Кожну розраховану точку нанесено на карту з літерою (A, B, …). Наведіть
    курсор — з'явиться червоний ✕ (клік — видалити); подвійний клік по мітці —
    перейменувати. «Очистити точки» — прибрати всі.

8. МАТРИЦЯ ТА РЕГРЕСІЯ
  • Вкладка «Матриця» показує повний план 3^N: кодовані фактори, справжні
    значення, τ і T_peak (°C / K). «Експорт матриці (CSV)» зберігає.
  • МНК-регресія T_peak(фактори) — в один рядок. «Копіювати регресію в LaTeX»
    копіює готове рівняння. На карту можна накласти скорочене T(x,y)
    (Plot style → «Регресія на карті») і скопіювати його.

9. ОФОРМЛЕННЯ ГРАФІКА (для статті)
Прокручувана смуга «Plot style» під графіком керує розміром тексту, відступами
підписів/цифр, рамкою параметрів (масштаб, позиція), колірною шкалою (позиція,
напис X, 180°), межами вікна (×T_s), радіус/діаметр, кількістю кольорів,
аспектом тощо. У 2D і 3D — ОКРЕМІ налаштування (крім температурного вікна); у 3D
додано підйом, азимут і щільність сітки. Збереження: PNG (растр) або PDF / SVG
(вектор) — з панелі над графіком.

10. ПРОЄКТИ, ЕКСПОРТ І ГАРЯЧІ КЛАВІШІ
  • Файл: Новий (Ctrl+N), Відкрити (Ctrl+O), Зберегти (Ctrl+S),
    Зберегти як (Ctrl+Shift+S). Незбережені правки програма перепитує під час
    створення/відкриття/закриття.
  • Знімок: PNG (Ctrl+Shift+P), PDF (Ctrl+Shift+D), SVG (Ctrl+Shift+G),
    сітка CSV (Ctrl+Shift+E).
  • Мова: Налаштування → Мова. Шифри матеріалів не перекладаються.

11. МОДЕЛЬ (стисло)
  p = F / (π·Rc²)
  τy(T) = (σ_ref/√3)·[1 − ((T−T0)/(T_sol−T0))^m]
  τc = min(μ·p, τy)                         (дотичне на контакті)
  Q  = (2/3)·π·τc·ω·Rc³                     (лише ТОРЕЦЬ контакту)
  Стік = 4·k_підк·Rc (+ 4·k_шайби·Rc у shouldered) + ρ_прут·cₚ·V̇  (адвекція подачі)
  T_peak = T0 + η·G(Pe)·Q / Стік            (самоузгоджено)
Два режими: ковзання (τc = μp, T лінійна за η) та залипання біля солідусу
(τc = τy, де η майже не впливає). Кондукція у пруток навмисно поглинута в η.
"""
